//! Validation of the full (non-low-rank) CD-G0W0 analytic gradient
//! (src/ri_gw/gw_grad.rs::GwCdGradEngine, the pyscf.gw.gw_cd_grad port):
//! analytic gradient vs central finite differences of the static-subtracted
//! continuous QP energy (H2 HOMO, H2O HOMO and LUMO).
...
fn qp_energy_displaced(scf_data: &SCF, atm: usize, comp: usize, h: f64, target: usize) -> f64 {
    ...
    cfg.qpe_tol = 1.0e-13;
    let engine = GwCdGradEngine::new(&new_scf, cfg);
...
fn check_gradient(scf_data: &SCF, target: usize, tol: f64) {
    ...
    let natm = engine.base.natm;
    let mut grad_fd = vec![0.0f64; natm * 3];
    let h = 1.0e-4;
    for atm in 0..natm {
        for comp in 0..3 {
            // the static-subtracted functional is smooth, so the plain
            // central difference is stable; still verify h-invariance and
            // keep the finer value
            let ep = qp_energy_displaced(scf_data, atm, comp, h, target);
            let em = qp_energy_displaced(scf_data, atm, comp, -h, target);
            let g = (ep - em) / (2.0 * h);
            let hh = h * 0.1;
            let ep2 = qp_energy_displaced(scf_data, atm, comp, hh, target);
            let em2 = qp_energy_displaced(scf_data, atm, comp, -hh, target);
            let g2 = (ep2 - em2) / (2.0 * hh);
            if std::env::var("REST_TEST_DBG").is_ok() {
                println!(
                    "[fd DBG] target={} atm={} comp={}: g(h={:.0e})={:+.10e} g(h={:.0e})={:+.10e}",
                    target, atm, comp, h, g, hh, g2
                );
            }
    ...
    for i in 0..grad_ana.len() {
        ...
        assert!(
            diff < tol,
            "CD-GW gradient component {} mismatch: ana={} fd={} (diff {:.3e} > {:.1e})",
            i,
            grad_ana[i],
            grad_fd[i],
            diff,
...
#[test]
fn cd_gw_grad_h2_homo_matches_finite_difference() {
...
#[test]
fn cd_gw_grad_h2o_homo_and_lumo_match_finite_difference() {
    let mut scf_data = build_mol(
        "H2O",
        "
    ...
    // symmetry-zero components carry CPHF Krylov noise ~5e-6, nonzero
    // components agree to ~1e-7
