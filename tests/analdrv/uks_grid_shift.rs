...
/// driver (see `rks_grid_shift::test_flag_off_matches_old_driver`).
#[test]
fn test_flag_off_matches_old_driver() {
    use pyrest::dft::numint_matmul::hess_uks::{make_hessian_setup_batched_uks, make_hessian_setup_becke_uks};
    use pyrest::dft::numint_matmul::nimatmul::{regroup_grids_by_atom, NIMatmul};
    use pyrest::ri_jk::util::get_cint_mol;
    ...
    for input in [INPUT_NH3_B3LYP, INPUT_NH3_TPSSH] {
        ...
        // per-spin dm0, the same way the UHessKSNIMatmul setup forms them
        let occidx_α = mo_occ[0].view().greater(0).into_vec();
        let mocc_α = mo_coeff[0].bool_select(-1, &occidx_α);
        let dm0α = &mocc_α % mocc_α.t();
        let occidx_β = mo_occ[1].view().greater(0).into_vec();
        let mocc_β = mo_coeff[1].bool_select(-1, &occidx_β);
        ...
        let (res_new, _) = make_hessian_setup_becke_uks(
            &cint_mol,
            &xc_func_list,
            &mut ni_new,
            dm0α.view(),
        ...
        let (res_old, _) = make_hessian_setup_batched_uks(
            &cint_mol,
            &xc_func_list,
            &mut ni_old,
            dm0α.view(),
