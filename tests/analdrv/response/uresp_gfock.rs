...
//! URespRIJK and URespKSNIMatmul, pinned two ways:
...
fn test_oh_uks_response_rdm_consistency() {
    ...
    rijk.make_response_preparation(&[mo_coeff[0].view(), mo_coeff[1].view()], &[mo_occ[0].view(), mo_occ[1].view()]);
    ...
    let resp_bra = rijk.get_response_bra(&[bra[0].view(), bra[1].view()]);
    let x = [&bra[0] % mocc[0].t(), &bra[1] % mocc[1].t()];
    let x_sym = [(&x[0] + &x[0].t()) * 0.5, (&x[1] + &x[1].t()) * 0.5];
    let resp_rdm = rijk.get_response_rdm(&[x_sym[0].view(), x_sym[1].view()]);
    for s in 0..2 {
    ...
    uks.make_response_preparation(&[mo_coeff[0].view(), mo_coeff[1].view()], &[mo_occ[0].view(), mo_occ[1].view()]);

    let resp_bra = uks.get_response_bra(&[bra[0].view(), bra[1].view()]);
    let resp_rdm = uks.get_response_rdm(&[x_sym[0].view(), x_sym[1].view()]);
    for s in 0..2 {
        let expect = resp_rdm[s].view() % mocc[s].view();
...
fn test_closed_shell_limit() {
    ...
    let resp_r = rrijk.get_response_rdm(x.view());
    let resp_u = urijk.get_response_rdm(&[x.view(), x.view()]);
    for s in 0..2 {
    ...
    rks.make_response_preparation(mo_coeff.view(), mo_occ.view());
    let occ_half = &mo_occ * 0.5_f64;
    uks.make_response_preparation(&[mo_coeff.view(), mo_coeff.view()], &[occ_half.view(), occ_half.view()]);

    let resp_r = rks.get_response_rdm(x.view());
    let resp_u = uks.get_response_rdm(&[x.view(), x.view()]);
    for s in 0..2 {
