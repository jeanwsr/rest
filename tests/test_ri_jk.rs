...
fn test_solved_j3c() {
    ...
    let j2c_decomp_option = J2CDecompOption { policy: J2CDecompPolicy::Cd, threshold: None, uplo: Upper, distributed: J2CDistributedMode::Auto };
    ...
    let j2c_decomp = get_j2c_decomp(
        &aux,
        &device,
        J2CDecompOption { policy: J2CDecompPolicy::Eig, threshold: Some(1e-13), uplo: Upper, distributed: J2CDistributedMode::Auto },
    );
    let j3c_solved = get_solved_j3c(j3c_, &j2c_decomp, false);
    let ptr_j3c_solved = j3c_solved.as_ptr();
    assert!(core::ptr::eq(ptr_j3c, ptr_j3c_solved));
