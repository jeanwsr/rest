...
use crate::solvers::nlfeast::{nlfeast as nlfeast_core, ContourNode, NlepOperator};
pub use crate::solvers::nlfeast::{qr_orthonormalise, NLFeastResult};
...
// ============================================================================
// BSE operator adapter for the context-independent NLFEAST kernel
// ============================================================================

/// BSE implementation of [`crate::solvers::nlfeast::NlepOperator`].
///
/// It supplies the three problem-specific operations the kernel needs:
///   * `projected_solve` — the BSE-projected NLEVP (TDA: A(λ); non-TDA:
///     (A(λ)+B(λ))(A−B) − λ²), including the frequency-dependent screening;
///   * `t_real`          — the real-axis residual operator T(λ)·x;
///   * `solve_shifted`   — the real-embedded complex shifted solve
///     T(z)·u = rhs via GMRES.
struct BseNlepOperator<'a> {
    scf_data: &'a SCF,
    qp_ctrl: &'a QuasiParticle,
    quasiparticle_energies: &'a Vec<f64>,
    occ_size: usize,
    vir_size: usize,
    n: usize,
    ri_vv_reshaped: &'a MatrixFull<f64>,
    ri_ov: &'a MatrixFull<f64>,
    ri_ov_response: &'a MatrixFull<f64>,
    ri_oo: &'a MatrixFull<f64>,
    ri_ov_b: MatrixFull<f64>,
    contour_nodes: &'a [ContourNodeData],
    energy_diag: Vec<f64>,
    tda: bool,
    gmres_restart: usize,
    gmres_max_it: usize,
...
impl NlepOperator for BseNlepOperator<'_> {
    fn dim(&self) -> usize {
        self.n
    }
    ...
    fn projected_solve(
        ...
        if self.tda {
            solve_projected_tda(
                self.scf_data, self.qp_ctrl, self.quasiparticle_energies,
                self.occ_size, self.vir_size, self.n,
                self.ri_vv_reshaped, self.ri_ov, self.ri_ov_response, self.ri_oo,
                q, lambda_init,
            )
        } else {
            solve_projected_bse(
    ...
    fn t_real(&self, lambda: f64, x: &[f64]) -> Vec<f64> {
        if self.tda {
            compute_real_tda_matvec(
                ...
                self.ri_ov_response, self.ri_oo, self.ri_vv_reshaped, self.ri_ov,
                lambda, x,
            )
        } else {
            compute_real_matvec(
                self.scf_data, self.qp_ctrl, self.quasiparticle_energies,
                self.occ_size, self.vir_size,
                self.ri_ov_response, self.ri_ov, self.ri_oo, self.ri_vv_reshaped,
                lambda, x,
            )
        }
    }

    fn solve_shifted(&self, node_index: usize, _z: Complex<f64>, rhs: &[f64]) -> Vec<f64> {
        let node = &self.contour_nodes[node_index];
        let n = self.n;

        let c_matvec: Box<dyn Fn(&[f64]) -> Vec<f64> + Send + Sync> = if self.tda {
            Box::new(make_shifted_tda_matvec(
                self.scf_data, self.qp_ctrl, n,
                self.ri_vv_reshaped, self.ri_ov, node,
            ))
        } else {
            Box::new(make_shifted_matvec(
                self.scf_data, self.qp_ctrl, n,
                self.ri_vv_reshaped, self.ri_ov, &self.ri_ov_b,
                node,
            ))
        };

        // 2×2 block-diagonal preconditioner:
        //   non-TDA: re = D² − (z_re² − z_im²), im = −2·z_re·z_im
        //   TDA:     re = D  −  z_re,          im = −z_im
        let z_re = node.z_re;
        let z_im = node.z_im;
        ...
        gmres(&c_matvec, rhs, self.gmres_restart, self.gmres_max_it, self.gmres_tol,
              false, Some(&gmres_precond))
    }
}
...
///
/// All problem-specific work is encapsulated in [`BseNlepOperator`]; the
/// subspace iteration itself lives in [`crate::solvers::nlfeast::nlfeast`].
///
/// # Arguments
///
...
/// * `_n_quad` — number of quadrature nodes (implied by `contour_nodes`)
...
pub fn nlfeast_bse(
    ...
    // lambda_init for the first projected solve: QP gaps closest to centre
    ...
    // ── Expose the contour nodes to the generic kernel ──
    let nodes: Vec<ContourNode> = contour_nodes.iter().map(|nd| ContourNode {
        z_re: nd.z_re,
        z_im: nd.z_im,
        w_re: nd.w_re,
        w_im: nd.w_im,
    ...
    nlfeast_core(&op, q_mat, lambda_init, &nodes, centre, radius, max_iter, tol)
}
