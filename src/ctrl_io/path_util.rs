...
pub fn get_rest_basis_dir(print_level:usize) -> Vec<String> {
    ...
    if let Some(val) = env::var("REST_BASIS_DIR").ok() {
        if !val.is_empty() {
            if path::Path::new(&val).is_dir() {
                ...
                println!("Add the REST_BASIS_DIR path ({}) to rest_basis_dir.", &val);
                // rest_basis_dir = val + "/";
    ...
    if let Some(p) = env::var("CONDA_PREFIX").ok() {
        ...
        debug!(print_level, "Detected CONDA_PREFIX: {}, inside_conda: {}", conda_prefix, inside_conda);
    }
    if inside_conda {
        ...
        if path::Path::new(&conda_basis_path).is_dir() {
            ...
            debug!(print_level, "The conda basis set path {} is not a valid directory.", conda_basis_path);
        }
    } 

    let default_basis_path = String::from(DEFAULT_BASIS_PATH);
    if path::Path::new(&default_basis_path).is_dir() {
        println!("Add the default path ({}) to rest_basis_dir.", &default_basis_path);
        ...
        if path::Path::new(&alt_basis_path).is_dir() {
            println!("Add the alternative default path ({}) to rest_basis_dir.", &alt_basis_path);
            rest_basis_dir.push(alt_basis_path.clone());
        } 
    } 
    if rest_basis_dir.is_empty() {
        println!("Cannot find the default basis set path.");
        println!("Please check if one of the following paths exist:");
        println!("  1. The path set by the REST_BASIS_DIR environment variable");
        println!("  2. The conda environment path: $CONDA_PREFIX/share/rest/basis-set-pool/ (if using conda)");
        println!("  3. The default installation path: {}", DEFAULT_BASIS_PATH);
        println!("  4. The alternative default path: $REST_HOME/rest/basis-set-pool/");
    }
    rest_basis_dir
}
