...
/// `[ctrl] check_stab` keyword. Valid values:
/// - `"internal"`: RHF/RKS singlet or UHF/UKS orbital Hessian.
/// - `"external"`: RHF→UHF (triplet) channel; RHF/RKS only.
/// - `"full"`: internal + external.
/// - `"auto"` (recommended): every check applicable to the reference type
///   (RHF/RKS: internal + external; UHF/UKS: internal, since UHF→GHF is not
///   yet implemented). Currently resolves to `"full"`.
///
...
pub fn stability(scf: &SCF, mode: &str) -> Result<StabilityReport, String> {
    let (nroots, tol) = match scf.mol.ctrl.tddft.as_ref() {
        Some(t) => (t.stability_nroots, t.stability_tol),
        None => {
