...
/// Unrestricted SCF hessian: assembles the driver (overlap/nuclear/core/electronic parts plus
/// the response object), evaluates the hessian and performs the vibrational analysis.
///
/// `resp_obj` is the SCF response object (composite [`URespSCF`], RI-JK plus XC parts) that
/// provides the CP-SCF solve for the relaxed density. It is not built here: initialize it
/// beforehand with [`crate::analdrv::response::uresp_interface::uscf_resp_interface`] on the
...
pub fn uscf_hess_interface<'a>(
    scf_data: &'a SCF,
    cfg: &AnalDrvNucgradCfg,
    resp_obj: &mut URespSCF<'a>,
    ...
    let mut hess_rijk_sr_obj = rsh.map(|(omega, factor_k_sr)| {
        ...
        if let Some((rimatr_sr, _, _)) = &scf_data.rimatr_sr {
            ...
            UHessRIJK::new_with_cderi(&mol_sr, &aux_sr, 0.0, factor_k_sr, cderi_sr, j2c_decomp_sr)
        } else {
            panic!(
    ...
    let mut hess_nimatmul_obj = scf_skeleton_nimatmul_uks(scf_data, cfg).map(|ni| {
        ...
        UHessKSNIMatmul::new(&mol, scf_xc_func_list_uks(scf_data), ni, grid_shift, verbose)
    });
