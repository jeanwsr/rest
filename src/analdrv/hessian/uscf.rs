...
///
/// The lifetime parameters mean the same as in
/// [`RHessSCF`](crate::analdrv::hessian::rscf::RHessSCF) (documented there): they decouple the
/// response object's borrow from its data, and the response object's data region from the hessian
/// component objects.
pub struct UHessSCF<'a, 'b, 'c> {
    pub mo_coeff: [Tsr; 2],
    pub mo_occ: [Tsr; 2],
    pub mo_energy: [Tsr; 2],
    pub ovlp_obj: &'c mut UHessOvlp,
    pub nuc_list: Vec<&'c mut dyn HessNucAPI>,
    pub core_list: Vec<&'c mut dyn UHessCoreAPI>,
    pub el_list: Vec<&'c mut dyn UHessElecInteractAPI>,
    /// Response (fock/response) object for the electron-interaction contributions (the composite
    /// `URespSCF` of the SCF); the CP-SCF machinery (`make_response_preparation`/`response_mo`)
    /// calls it, separate from the skeleton-hessian objects in `el_list`.
    pub resp: &'b mut URespSCF<'a>,
...
impl<'a, 'b, 'c> UHessSCF<'a, 'b, 'c> {
    #[allow(clippy::too_many_arguments)]
    pub fn new(
        mo_coeff: [Tsr; 2],
        mo_occ: [Tsr; 2],
        mo_energy: [Tsr; 2],
        ovlp_obj: &'c mut UHessOvlp,
        nuc_list: Vec<&'c mut dyn HessNucAPI>,
        core_list: Vec<&'c mut dyn UHessCoreAPI>,
        el_list: Vec<&'c mut dyn UHessElecInteractAPI>,
        resp: &'b mut URespSCF<'a>,
    ...
    pub fn make_response_preparation(&mut self) {
        ...
        let mo_energy = [self.mo_energy[0].view(), self.mo_energy[1].view()];
        self.resp.make_cpscf_preparation(&mo_coeff, &mo_occ, &mo_energy);
    ...
    pub fn finalize_cpscf(
        ...
        let last_resp = self.resp.response_mo(mo1);
    ...
    pub fn make_cpscf_hess(&mut self) -> Tsr {
        // cached on first call; repeated calls directly return the stored result
        ...
        let de_cpscf = self.get_cpscf_hess(&f1mo, &s1mo, &mo1, &mo_e1);
