...
- 并行一般针对辅助基进行；每个并行线程会产生额外的 $\frac{1}{2} n_\mathrm{basis}^2$ 存储消耗。
...
这里计算，唯一的损耗是每次 SCF 迭代计算时，需要重新计算一遍 3c-2e ERI $g_{\mu \nu, P}$，而这是内存较小时不可能避免的。
...
尽管公式表达式与 (eq.6-8) 是非常接近的，但在实现的具体细节上，可能会有较大的差别。

- 对于基于辅助基的分批，Cholesky 分解会比较麻烦，因为更合适的办法是直接求 2c-2e ERI $J_{PQ}$ 的逆；这带来的代价是，会有些许精度损失。
- 实际计算中，我们应该能利用辅助基的对称性；下式是对 (eq.8) 的改动：

    $$
    K_{\mu \nu} [\mathbf{D}] = \sum_{i} \sum_{P < Q} (2 - \delta_{PQ}) g_{\mu i, P} (\mathbf{J}^{-1})_{PQ} g_{\nu i, Q}
    $$
