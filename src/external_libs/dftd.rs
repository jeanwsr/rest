...
pub fn dftd(scf_data: &SCF) -> Option<(f64, Option<Vec<f64>>, Option<Vec<f64>>)> {
    ...
    if disp_from_parse_xc {
        ...
        match disp.func.to_lowercase().as_str() {
            "dftd3" => {
                return Some(dftd3_atm_from_parse_xc(scf_data, &disp));
            },
            "dftd4" => {
                return Some(dftd4_atm_from_parse_xc(scf_data, &disp));
            },
            _ => {
    ...
    if disp_from_ctrl {
        ...
        if let Some(tmp_emprical) = &scf_data.mol.ctrl.empirical_dispersion {
            if tmp_emprical == "d3" || tmp_emprical == "d3bj" {
                return Some(dftd3_atm_from_scf(scf_data));
            } else if tmp_emprical == "d4" {
                return Some(dftd4_atm_from_scf(scf_data));
            } else {
                panic!(
